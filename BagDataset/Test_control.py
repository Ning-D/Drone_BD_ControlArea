...
class BagDataset2(Dataset):

    def __init__(self, transform=None):
        self.transform = transform
        
    def __len__(self):
        return len(pd.read_csv('Test_samples_control.csv', header=None))


    def __getitem__(self, idx):
    
    
        img_name = pd.read_csv('Test_samples_control.csv', header=None).loc[idx][0]
