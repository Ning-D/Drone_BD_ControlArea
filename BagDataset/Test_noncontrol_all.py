...
class BagDataset(Dataset):
    ...
    def __len__(self):
        return len(pd.read_csv('Test_samples_nocontrol_all.csv', header=None))


    def __getitem__(self, idx):
    
    
        img_name = pd.read_csv('Test_samples_nocontrol_all.csv', header=None).loc[idx][0]
        ...
        Loc=[] 
        Loc.append([Rx[0],Ry[0]])
        Loc.append([Sx[0],Sy[0]])
        Loc.append([locx[0],locy[0]])
        #Loc.append(imgC[0][Rx[0],Ry[0])
        #print()
        PATH = './Optimal/'+img_name.split('_')[0]+'/'+img_name.split('_')[1]
        if not os.path.exists(PATH):
            os.makedirs(PATH)
        
        np.savetxt('./Optimal/'+img_name.split('_')[0]+'/'+img_name.split('_')[1]+'/Nocontrol.txt', Loc,   fmt="%s") 
